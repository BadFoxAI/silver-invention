// Ensure Babylon.js classes are available (they are loaded globally via CDN scripts)
// You could also use import maps or a bundler like Webpack/Vite for explicit imports
...
let xrHelper = null; // Store XR helper reference
...
try {
    ...
    throw e; // Stop execution if engine fails
}
...
const createScene = async () => {
    ...
    scene = new BABYLON.Scene(engine); // Assign to global scene variable
    ...
    try {
        environmentHelper = scene.createDefaultEnvironment({
            ...
            createGround: true,
            groundSize: 100,
            // Let's explicitly set groundColor here too, although it might be overridden by the material below
        ...
        if (environmentHelper) {
            ...
            if (environmentHelper.ground) {
                ground = environmentHelper.ground;
                ground.name = "environmentGround";
                ground.checkCollisions = true;
                ground.receiveShadows = true;

                // *** FLOOR FIX ATTEMPT: Explicitly assign/check material ***
                if (!ground.material) {
                    ...
                    groundMat.diffuseColor = new BABYLON.Color3(0.5, 0.55, 0.5); // Same as groundColor option
                    groundMat.specularColor = new BABYLON.Color3(0.1, 0.1, 0.1);
                    ground.material = groundMat;
                } else {
                    // Ensure existing material is opaque if it exists
                    ground.material.alpha = 1.0;
                    console.log("Environment ground material found:", ground.material.name);
                }
                // **********************************************************
                ...
                throw new Error("Environment helper created but ground is missing.");
            }
        } else {
            throw new Error("createDefaultEnvironment returned null.");
        }

    } catch (envError) {
        console.error("Error creating default environment:", envError);
        statusElement.innerText = "Env Error. Using Fallback.";
        scene.createDefaultLight();
        if (!ground) {
            ground = BABYLON.MeshBuilder.CreateGround("errorFallbackGround", { width: 100, height: 100 }, scene);
            // Assign a visible material to the fallback ground
            let fallbackMat = new BABYLON.StandardMaterial("fallbackGroundMat", scene);
            fallbackMat.diffuseColor = new BABYLON.Color3(0.6, 0.6, 0.6); // Grey fallback
    ...
    dirLight.position = new BABYLON.Vector3(50, 80, 40);
    ...
    boxMaterial.diffuseColor = new BABYLON.Color3(0.4, 0.4, 0.9);
    boxMaterial.specularPower = 32;
    const commonPhysicsProps = { restitution: 0.4, friction: 0.6 };
    ...
    for (let i = 0; i < objectCount; i++) {
        ...
        if (type === 'sphere') {
            mesh = BABYLON.MeshBuilder.CreateSphere(`sphere${i}`, { diameter: size }, scene);
            mesh.material = sphereMaterial; impostorType = BABYLON.PhysicsImpostor.SphereImpostor;
        } else {
            mesh = BABYLON.MeshBuilder.CreateBox(`box${i}`, { size: size }, scene);
            mesh.material = boxMaterial; mesh.rotation.x = Math.random() * Math.PI; mesh.rotation.y = Math.random() * Math.PI;
    ...
    if (navigator.xr) {
        try {
            ...
            if (xrHelper && xrHelper.baseExperience) {
                ...
                statusElement.innerText = "WebXR Ready!";

                xrHelper.baseExperience.onStateChangedObservable.add((state) => {
                    console.log("WebXR State Changed:", BABYLON.WebXRState[state]);
                    if (state === BABYLON.WebXRState.IN_XR) { infoElement.style.display = 'none'; crosshairElement.style.display = 'none'; }
                });

                xrHelper.baseExperience.onInitialCameraReadyObservable.addOnce((xrCamera) => {
                    console.log("WebXR Camera is ready.");
                });

            } else {
                console.warn("WebXR Base Experience could not be initialized.");
                statusElement.innerText = "WebXR Failed (Check HTTPS/Device)";
            }
        } catch (xrError) {
            console.error("Error during WebXR initialization:", xrError);
            statusElement.innerText = `WebXR Error: ${xrError.message}`;
        }
    } else {
...
const setupControls = () => {
    ...
    const pointerLockChange = () => {
        ...
        if (element === canvas && !isInVR) {
            isPointerLocked = true; infoElement.style.display = 'none'; crosshairElement.style.display = 'block'; console.log("Pointer locked.");
        } else {
            isPointerLocked = false;
            if (!isInVR) { infoElement.style.display = 'block'; }
            else { infoElement.style.display = 'none'; }
            crosshairElement.style.display = 'none'; console.log("Pointer unlocked or in VR.");
        }
    };
...
(async () => {
    try {
        await createScene(); // Wait for scene creation to complete

        if (!scene) {
            throw new Error("Scene creation failed unexpectedly.");
        }

        console.log("Starting render loop.");
        engine.runRenderLoop(() => {
            if (scene) { // Ensure scene exists before rendering
               scene.render();
            }
        });
